...
export function PlannerEmptyState({ actionLabel, children, compact = false, description, onAction, title, transition = "instant" }: PlannerEmptyStateProps) {
  return (
    ...
    >
      <Empty className={compact ? "gap-2 px-3 py-4" : "mt-6 min-h-48 justify-center gap-4 px-4 py-6 sm:min-h-64"}>
        <EmptyHeader>
          <EmptyMedia variant="icon">{children}</EmptyMedia>
          <EmptyTitle className={compact ? "text-task font-medium text-foreground" : "font-heading text-2xl font-semibold leading-tight tracking-tight text-foreground"}>{title}</EmptyTitle>
