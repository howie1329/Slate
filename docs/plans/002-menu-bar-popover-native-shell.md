...
- Add a dedicated menu-bar template icon. Keep the full-color application and bundle icons separate from the tray mark; the canonical source and generated macOS outputs are documented in `docs/macos-distribution.md`. A large colored app icon is not a tray icon. A small in-memory RGBA template is preferred when it avoids another image-decoding dependency.
